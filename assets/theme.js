(() => {
  ...
  var ToggleButton = class extends HTMLButtonElement {
    ...
    set isExpanded(value) {
      ...
      if (
        this.controlledElement.tagName === "PRODUCT-IMAGE-ZOOM" &&
    ...
    syncPath() {
      ...
      if (someElsAreVisible() && pathStart < pathEnd) {
        if (pathStart !== this.lastPathStart || pathEnd !== this.lastPathEnd) {
          const dashArray = `1 ${pathStart} ${pathEnd - pathStart
            } ${pathLength}`;
...
`;
  ...
  var QuantitySelector = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      const productFormId = this.querySelector(
  ...
  var VideoCarouselItem = class extends CustomHTMLElement {
    connectedCallback() {
      this.videoContainer = this.querySelector(
      ...
      this.closeButton.addEventListener("click", () => {
        ...
        if (this.hiddenOnClose) {
          localStorage.setItem(
            `theme:hidden:shopify-section-${this.sectionId}`,
            true,
          );
        }
      });
  ...
  var PredictiveSearchDrawer = class extends DrawerContent {
    ...
    async _doPredictiveSearch(term) {
      const response = await fetch(
        ...
        }?q=${encodeURIComponent(term)}&resources[limit]=10&resources[type]=${window.themeVariables.settings.searchMode
  ...
  var PhotoSwipeUi = class {
    ...
    init() {
      ...
      this.photoSwipeInstance.items.forEach((item, index) => {
        dotsNavHtml += `
        <button class="dots-nav__item tap-area" ${index === 0 ? 'aria-current="true"' : ""
          } data-action="pswp-move-to">
  ...
  var PaymentContainer = class extends HTMLElement {
    ...
    _updateAddToCartButton(variant) {
      ...
      const addToCartButtonIcon =
        addToCartButtonElement.querySelector(".loader-button__icon")
          ?.outerHTML || "";
      ...
      if (addToCartButtonElement.getAttribute("is") === "loader-button") {
        addToCartButtonElement.firstElementChild.innerHTML =
          addToCartButtonIcon + addToCartButtonText;
      } else {
  ...
  var ProductForm = class extends HTMLFormElement {
    ...
    async _onSubmit(event, isCheckout) {
      ...
      let freeGiftProduct;
      let freeGiftType;
      const isFreeGiftPopup =
        this.querySelector('input[name="free_gift_product_popup"]')?.value ===
        "true";
      if (this.querySelector('input[name="free_gift_product_id"]')) {
        freeGiftProduct = {
          id: this.querySelector('input[name="free_gift_product_id"]').value,
          quantity: this.querySelector(
            'input[name="free_gift_product_quantity"]',
          ).hasAttribute("fixed-quantity")
            ? Number(
              this.querySelector('input[name="free_gift_product_quantity"]')
                .value,
            )
            : Number(
              this.querySelector('input[name="free_gift_product_quantity"]')
  ...
  var QuickBuyDrawer = class extends DrawerContent {
    ...
    async _load() {
      ...
      const recommendedElement = this.querySelector(
        ".product-form__product-recommendations",
      );
  ...
  var ProductVariants = class extends CustomHTMLElement {
    ...
    _onDynamicTheme() {
      ...
      const sectionEle = document.querySelector(
        ".shopify-section--main-product",
      );
      const dynamicThemeVariablesEle = sectionEle.querySelector(
      ...
      const dynamicThemeVariables = {
        settings: {
          ...
          enable_announcement_bar: dynamicThemeVariablesEle.hasAttribute(
            "enable-announcement-bar",
          ),
          enable_footer: dynamicThemeVariablesEle.hasAttribute("enable-footer"),
        },
      };
      if (!dynamicThemeVariables || !dynamicThemeVariables.settings.enabled)
        return;
      ...
      if (sectionEle) {
        this.selectedVariant.options.forEach((option) => {
          const label =
            sectionEle &&
            sectionEle.querySelector(
              `.color-swatch-list .color-swatch__radio[value='${option}'] ~ label`,
            );
      ...
      if (colorSwatchLabel) {
        ...
        const productFormBuyButtons = document.querySelector(
        ...
        if (textColor) {
          ...
          if (
            announcementBar &&
            dynamicThemeVariables.settings.enable_announcement_bar
          ) {
            announcementBar.style.setProperty("--text-color", textColor);
          }
          if (productFormBuyButtons) {
            productFormBuyButtons.style.setProperty(
  ...
  var ProductItem = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      if (this.hasAttribute("truncate-variant-images")) {
        this.delegate.on(
          "click",
          ".product-item-meta__swatch-list .more-options",
          () => {
    ...
    async _rerenderCart(event) {
      ...
      setTimeout(
        ...
        ((_a = event == null ? void 0 : event.detail) == null
          ? void 0
          : _a.replacementDelay) || this.nextReplacementDelay,
      );
      this.nextReplacementDelay = 0;
    }
    async attributeChangedCallback(name, oldValue, newValue) {
  ...
  var LineItemQuantity = class extends CustomHTMLElement {
    ...
    async _updateFromLink(link) {
      ...
      if (window.themeVariables.settings.pageType === "cart") {
        ...
        section
          .querySelectorAll(".line-item .line-item__loader")
          .forEach((item) => {
            if (!item) return;
            item.hidden = false;
      ...
      if (window.themeVariables.settings.pageType === "cart") {
        const response = await fetch(
          `${window.themeVariables.routes.rootUrl}?section_id=${sectionId}`,
          {
            credentials: "same-origin",
            method: "GET",
          },
  ...
  var ReviewLink = class extends HTMLAnchorElement {
    ...
    _onClick() {
      ...
      if (window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
        ...
        document
          .querySelector(
            `[aria-controls="${shopifyReviewsElement.closest(".product-tabs__tab-item-wrapper")
              .id
            }"]`,
          )
          .click();
  ...
  var ZalifyCountdown = class extends CustomHTMLElement {
    ...
    _run() {
      ...
      var endTime = new Date(
        rootEl.dataset.endTime || initialEndTime,
      ).getTime();
      if (isNaN(endTime)) {
        return;
      }
      ...
      if (timeLeft > 0) {
        ...
        var hours = this._transformNumber(
          ...
          entries.forEach((entry) => {
            if (entry.target === paymentContainerElement) {
              ...
              this._isPaymentContainerPassed =
                ...
                boundingRect.top < stickyHeaderOffset;
            }
          });
  ...
  var ProductRecommendationList = class extends CustomHTMLElement {
    constructor() {
      ...
      this.productListInner = this.querySelector(
        ".product-recommendation-list__inner",
      );
      this.productItems = Array.from(this.querySelectorAll("product-item"));
    }
    connectedCallback() {
  ...
  var StoriesSlider = class extends HTMLElement {
    connectedCallback() {
      ...
      this.listItemElements = Array.from(
        this.querySelectorAll(".stories-slider__item"),
      );
      this.isScrolling = false;
      if (this.listItemElements.length > 1) {
        if (Shopify.designMode) {
          this.addEventListener("shopify:block:select", (event) => {
            this.select(
              [...event.target.parentNode.parentNode.children].indexOf(
  ...
  var FreeGiftProductPopup = class extends ModalContent {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      switch (name) {
        case "open":
          if (this.open) {
            this.querySelector(".product-free-gift-popup__quantity").value =
  ...
  var FreeGiftWrapper = class extends CustomHTMLElement {
    connectedCallback() {
      if (document.querySelector('[block-type="free-gift"]')) {
        ...
        ).outerHTML = document.querySelector(
          'input[name="free_gift_product_id"]',
        ).outerHTML;
        productForm.querySelector(
          'input[name="free_gift_product_quantity"]',
  ...
  var CartApplyCode = class extends CustomHTMLElement {
    ...
    _onValueInput(e) {
      document.querySelector('#mini-cart-form input[name="discount"]').value =
        e.target.value;
    }
  };
  ...
  const ProductBundles = class extends HTMLElement {
    ...
    async _handleAddToCart(event) {
      ...
      const atcBtn = formElement.querySelector(
        "[data-product-add-to-cart-button]",
      );
      const quantity = event.target.dataset.quantity;
      const inputElement = document.querySelector(
        `.product-form .quantity-selector__input[form="${this.formId}"]`,
      );
  ...
  (() => {
    document.addEventListener("variant:changed", async function (event) {
      ...
      const bundleElement = formId
        ? document.querySelector(`.product-form__bundle[form-id="${formId}"]`)
        : null;
      const bundleWrapper = bundleElement
        ? bundleElement.closest(".product-form__bundles")
        : null;
      ...
      bundleElement.outerHTML = fakeDiv.querySelector(
        `.product-form__bundle[form-id="${formId}"]`,
      ).outerHTML;
    });
  })();
  (() => {
    document.addEventListener("cart:show-and-fetch", async function (event) {
      const response = await fetch(
        `${window.themeVariables.routes.cartUrl}.js`,
      );
      const cartContent = await response.json();
  ...
  (() => {
    document.addEventListener("variant:changed", async function (event) {
      ...
      const paypalCheckoutContainer = formId
        ? document.querySelector(
          `.paypal-button-container[form-id="${formId}"]`,
        )
        : null;
