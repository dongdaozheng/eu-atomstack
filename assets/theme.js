(() => {
  ...
  var OpenableElement = class extends CustomHTMLElement {
    ...
    constructor() {
      ...
      if (Shopify.designMode) {
        if (this.hasAttribute('data-shopify-editor-block')) {
          this.rootDelegate.on('shopify:block:select', (event) => {
            if (!this.manually) {
              filterShopifyEvent(event, this, () => (this.open = true));
            }
          });
          this.rootDelegate.on('shopify:block:deselect', (event) =>
    ...
    get manually() {
      return this.hasAttribute('manually');
    }
    get requiresLoading() {
  ...
  var ToggleButton = class extends HTMLButtonElement {
    ...
    set isExpanded(value) {
      ...
      this.mobileNavigation.style.display = value ? 'block' : 'none';

      const parentElement = this.closest('product-media');

      if (this.controlledElement.tagName === 'PRODUCT-IMAGE-ZOOM' && parentElement !== undefined) {
        if (value) {
          parentElement.classList.add('product__media--expanded');
        } else {
          parentElement.classList.remove('product__media--expanded');
        }
      }
    }
...
`;
  ...
  var CountrySelector = class extends HTMLSelectElement {
    connectedCallback() {
      ...
      const provinceSelectElement =
        this.provinceElement.tagName === 'SELECT' ? this.provinceElement : this.provinceElement.querySelector('select');
  ...
  var ModalContent = class extends OpenableElement {
    connectedCallback() {
      super.connectedCallback();
      if (this.appearAfterDelay && !this.manually && !(this.onlyOnce && this.hasAppearedOnce)) {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      super.attributeChangedCallback(name, oldValue, newValue);
      this.dispatchEvent(
        new CustomEvent('modal:attribute:changed', {
          bubbles: true,
          detail: { variant: this.open },
        }),
      );
      switch (name) {
        case 'open':
  ...
  var NativeVideo = class extends HTMLElement {
    ...
    _replaceContent() {
      const template2 = this.querySelector('template'),
        node = this.querySelector('template').content.firstElementChild.cloneNode(true);
      if (this.autoPlay) {
        template2.replaceWith(node);
      } else {
        this.innerHTML = '';
  ...
  var QuantitySelector = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      const productFormId = this.querySelector('.quantity-selector__input').getAttribute('form');
      const mainQuantitySelector = document.querySelector(
        `.product-form .quantity-selector__input[form='${productFormId}']`,
      );
      const stickyQuantitySelector = document.querySelector(
        `.product-sticky-form .quantity-selector__input[data-form='${productFormId}']`,
      );
      if (stickyQuantitySelector) {
        stickyQuantitySelector.removeAttribute('form');
      }

      this.delegate.on('click', 'button:first-child', () => {
        const quantity = this.inputElement.quantity;
        if (stickyQuantitySelector) {
          stickyQuantitySelector.quantity = quantity - 1;
        }
        if (mainQuantitySelector) {
          mainQuantitySelector.quantity = quantity - 1;
        }
      });
  ...
  // flip element
  var FlipText = class extends CustomHTMLElement {
    async connectedCallback() {
      await customElements.whenDefined('flip-text');
      this.items = Array.from(this.querySelectorAll('flip-text-item'));
      this.hasPendingTransition = false;
      if (this.autoPlay) {
        this.delegate.on('announcement-bar:content:open', this._pausePlayer.bind(this));
        this.delegate.on('announcement-bar:content:close', this._startPlayer.bind(this));
  ...
  window.customElements.define('flip-text', FlipText);

  var FlipTextItem = class extends CustomHTMLElement {
    connectedCallback() {}
    get index() {
      return [...this.parentNode.children].indexOf(this);
    }
  ...
  window.customElements.define('flip-text-item', FlipTextItem);

  var VideoCarouselItem = class extends CustomHTMLElement {
    connectedCallback() {
      this.videoContainer = this.querySelector('.video-carousel-item__image-container');
      this.modal = this.querySelector('modal-content');
      this.video =
        this.querySelector('.video-carousel-popup native-video') ||
        this.querySelector('.video-carousel-popup external-video') ||
        null;

      if (this.hasPopup && this.videoContainer && this.modal) {
        this.videoContainer.addEventListener('click', () => (this.modal.open = true));
      }
      if (this.video) {
        this.addEventListener('modal:attribute:changed', (event) => {
    ...
    get hasPopup() {
      return this.hasAttribute('has-popup');
    }
  };
  window.customElements.define('video-carousel-item', VideoCarouselItem);

  var VideoBubble = class extends CustomHTMLElement {
    connectedCallback() {
      if (!this.hasHidden) {
        this.removeAttribute('hidden');
      }

      this.closeButton = this.querySelector('.bubble__close-button');
      this.modal = this.querySelector('modal-content');
      this.videoContainer = this.querySelector('.video-wrapper');
      this.video =
        this.querySelector('.video-bubble-v2__popup native-video') ||
        this.querySelector('.video-bubble-v2__popup external-video') ||
        null;

      this.closeButton.addEventListener('click', () => {
        this.setAttribute('hidden', '');
        if (this.hiddenOnClose) {
          localStorage.setItem(`theme:hidden:shopify-section-${this.sectionId}`, true);
        }
      });
    ...
    get hasHidden() {
      if (this.hiddenOnClose) {
        return localStorage.getItem(`theme:hidden:shopify-section-${this.sectionId}`) === 'true';
      }
      return false;
    }
    get hasPopup() {
    ...
    get hiddenOnClose() {
      return this.hasAttribute('hidden-on-close');
    }
    get sectionId() {
      return this.getAttribute('section-id');
    }
  };
  window.customElements.define('video-bubble', VideoBubble);
  ...
  var DesktopNavigation = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.li = this.querySelectorAll('.nav-bar>li');
      this.activeIndex = 0;
      this.init();
    }
    init() {
      this.li.forEach((item, index) => {
        this.productSidebar(item);
    ...
    productSidebar(el) {
      el?.querySelectorAll('.left .list')?.forEach((item) => {
        item?.querySelectorAll('li')?.forEach((li, i) => {
          li.addEventListener('mouseenter', () => {
            Array.from(el?.querySelectorAll('.product-wrapper'))?.filter((subItem, j) => {
    ...
    initItem() {
      this.li.forEach((item) => item.classList.remove('active'));
    }
  };
  ...
  var MobileNavigation = class extends DrawerContent {
    connectedCallback() {
      this.menu = this.querySelectorAll('.nav-bar > li .menu');
      this.bar = this.querySelectorAll('.product-bar');
      this.support = this.querySelectorAll('.support .container p');
      this.handleMenu();
      this.handleBar();
      this.handleSupport();
    }
    ...
    handleSupport() {
      let activeIndex = -1;
      this.support.forEach((item, index) => {
        item.addEventListener('click', () => {
          activeIndex = index === activeIndex ? -1 : index;
          this.init(this.support);
          if (activeIndex !== -1) {
            item.classList.add('active');
          }
        });
  ...
  var PaymentContainer = class extends HTMLElement {
    ...
    _updateAddToCartButton(variant) {
      ...
      const addToCartButtonIcon = addToCartButtonElement.querySelector('.loader-button__icon')?.outerHTML || '';
      ...
      if (addToCartButtonElement.getAttribute('is') === 'loader-button') {
        addToCartButtonElement.firstElementChild.innerHTML = addToCartButtonIcon + addToCartButtonText;
      } else {
        addToCartButtonElement.innerHTML = addToCartButtonIcon + addToCartButtonText;
      }
    }
  ...
  var ProductForm = class extends HTMLFormElement {
    ...
    onCheckout() {
      const checkoutEl = this.querySelector('button[checkout]');
      if (!checkoutEl) {
        return;
      }
      checkoutEl.addEventListener('click', (event) => {
        checkoutEl.setAttribute('disabled', 'disabled');
        checkoutEl.setAttribute('aria-busy', 'true');
        this._onSubmit(event, true).finally(() => {
          checkoutEl.removeAttribute('disabled');
          checkoutEl.setAttribute('aria-busy');
        });
      });
    }
    async _onSubmit(event, isCheckout) {
      event.preventDefault();
      ...
      let freeGiftProduct;
      let freeGiftType;
      const isFreeGiftPopup = this.querySelector('input[name="free_gift_product_popup"]')?.value === 'true';
      if (this.querySelector('input[name="free_gift_product_id"]')) {
        freeGiftProduct = {
          id: this.querySelector('input[name="free_gift_product_id"]').value,
          quantity: this.querySelector('input[name="free_gift_product_quantity"]').hasAttribute('fixed-quantity')
            ? Number(this.querySelector('input[name="free_gift_product_quantity"]').value)
            : Number(this.querySelector('input[name="free_gift_product_quantity"]').value) *
              Number(productForm.get('quantity')),
        };
        freeGiftType = this.querySelector('input[name="free_gift_type"]').value;
      }
      if (freeGiftProduct?.id && freeGiftType === 'line_item') {
        productForm.set('items[1]id', freeGiftProduct.id);
        productForm.set('items[1]quantity', freeGiftProduct.quantity || 1);
      }
      ...
      if (response.ok) {
        ...
        if (isFreeGiftPopup) {
          if (isCheckout) {
            ...
            this.dispatchEvent(
              new CustomEvent('product:free-gift:added', {
                bubbles: true,
                detail: {
        ...
        } else if (freeGiftProduct?.id && freeGiftType !== 'line_item') {
          this.dispatchEvent(
            new CustomEvent('product:free-gift:show', {
              bubbles: true,
              detail: {
                drawerEl: this.closest('.drawer'),
                sections: responseJson['sections'],
                quantity: productForm.get('quantity'),
                productId: productForm.get('id'),
              },
            }),
          );
        } else {
          document.documentElement.dispatchEvent(
            new CustomEvent('cart:show-and-fetch', {
              bubbles: true,
              detail: {
                drawerEl: this.closest('.drawer'),
                sections: responseJson['sections'],
  ...
  var ProductMeta = class extends HTMLElement {
    ...
    get includeVariantName() {
      return this.hasAttribute('include-variant-name');
    }
    get priceClass() {
    ...
    _onVariantChanged(event) {
      ...
      if (this.includeVariantName) {
        this._updateTitle(event.detail.variant);
      }
    }
    _updateLabels(variant) {
      ...
      if (!variant) {
        ...
        if (!variant['available']) {
          ...
          productLabelList.innerHTML = `<span class="label">${window.themeVariables.strings.collectionDiscount.replace(
            '@savings@',
            savings,
          )}</span>`;
        }
    ...
    _updatePrices(variant) {
      ...
      if (!variant) {
        ...
        if (variant['compare_at_price'] > variant['price']) {
          ...
          }</span><span class="price_compare">${formatMoney(variant['compare_at_price'])}</span></span>`;
        } else {
          productPrices.innerHTML += `<span class="price price price--highlight ${this.priceClass}" data-price="${variant['price']}"><span class="visually-hidden">${
            window.themeVariables.strings.productSalePrice
          }</span> <span class="price_highlight">${formatMoney(variant['price'])}</span></span>`;
        }
        if (variant['unit_price_measurement']) {
          ...
          productPrices.innerHTML += `
          <div class="price text--subdued price price--highlight ${this.unitPriceClass}">
    ...
    _updateTitle(variant) {
      this.querySelector('.product-meta__title').innerText = variant.name;
    }
  };
  ...
  var QuickBuyDrawer = class extends DrawerContent {
    ...
    async _load() {
      ...
      // Remove custom recommended products if exists.
      const recommendedElement = this.querySelector('.product-form__product-recommendations');
      if (recommendedElement) recommendedElement.remove();

      // Remove product bundles if exists.
      const productBundles = this.querySelector('.product-form__bundle');
      if (productBundles) productBundles.remove();
    }
    _onVariantChanged(event) {
      const variant = event.detail.variant;
      if (variant) {
  ...
  var ProductVariants = class extends CustomHTMLElement {
    ...
    _onDynamicTheme() {
      ...
      const sectionEle = document.querySelector('.shopify-section--main-product');
      const dynamicThemeVariablesEle = sectionEle.querySelector('.dynamic-theme-variables');

      const dynamicThemeVariables = {
        settings: {
          enabled: dynamicThemeVariablesEle.hasAttribute('enabled'),
          enable_announcement_bar: dynamicThemeVariablesEle.hasAttribute('enable-announcement-bar'),
          enable_footer: dynamicThemeVariablesEle.hasAttribute('enable-footer'),
        },
      };
      if (!dynamicThemeVariables || !dynamicThemeVariables.settings.enabled) return;

      let colorSwatchLabel = null;
      if (sectionEle) {
        this.selectedVariant.options.forEach((option) => {
          const label =
            sectionEle &&
            sectionEle.querySelector(`.color-swatch-list .color-swatch__radio[value='${option}'] ~ label`);

          if (label) {
            colorSwatchLabel = label;
          }
        });
      }
      ...
      if (colorSwatchLabel) {
        ...
        const footerElement = document.querySelector('#shopify-section-footer .footer');
        const productFormBuyButtons = document.querySelector('.product-form__buy-button');
        const textColor = colorSwatchLabel.dataset.textColor;
        const bgColor = colorSwatchLabel.dataset.bgColor;

        if (textColor) {
          root.style.setProperty('--primary-button-text-color', textColor);
          root.style.setProperty('--product-on-sale-text-color', textColor);
          if (announcementBar && dynamicThemeVariables.settings.enable_announcement_bar) {
            announcementBar.style.setProperty('--text-color', textColor);
          }
          if (productFormBuyButtons) {
            productFormBuyButtons.style.setProperty('--primary-button-text-color', textColor);
          }
          if (footerElement && dynamicThemeVariables.settings.enable_footer) {
            footerElement.style.setProperty('--heading-color', textColor);
            footerElement.style.setProperty('--text-color', textColor);
            footerElement.style.setProperty('--border-color', textColor);
          }
        }
        if (bgColor) {
          root.style.setProperty('--primary-button-background', bgColor);
          root.style.setProperty('--product-bundle-border-color', bgColor);
          root.style.setProperty('--product-on-sale-background', bgColor);
          if (announcementBar && dynamicThemeVariables.settings.enable_announcement_bar) {
            announcementBar.style.setProperty('--section-background', bgColor);
          }
          if (productFormBuyButtons) {
            productFormBuyButtons.style.setProperty('--primary-button-background', bgColor);
          }
          if (footerElement && dynamicThemeVariables.settings.enable_footer) {
            footerElement.style.setProperty('--background', bgColor);
          }
        }
      }
    }
  ...
  var ProductItem = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.delegate.on('click', '.showcase-item__primary-button', () => {
        const modalElement = this.querySelector('modal-content');
        if (modalElement) {
          modalElement.open = true;
        }
      });
      if (this.hasAttribute('truncate-variant-images')) {
        this.delegate.on('click', '.product-item-meta__swatch-list .more-options', () => {
          this.querySelectorAll('.color-swatch').forEach((element) => {
            element.removeAttribute('hidden');
          });
          const button = this.querySelector('.product-item-meta__swatch-list .more-options');
          button.remove();
        });
      }
    }
    ...
    _onColorSwatchHovered(event, target) {
      ...
      if (input && input.hasAttribute('data-variant-featured-media')) {
        const newImage = this.primaryImageList.find(
  ...
  var ProductFacet = class extends CustomHTMLElement {
    ...
    async _rerender(event) {
      ...
      try {
        ...
        if (!fakeDiv.querySelector('#facet-main')) {
          document.querySelector('.shopify-section--main-collection').innerHTML = fakeDiv.innerHTML;
          return;
        }
    ...
    async _rerenderCart(event) {
      ...
      setTimeout(
        async () => {
          ...
          // snippets/paypal-checkout.liquid
          // Doc: https://developer.paypal.com/sdk/js/reference/
          const checkoutBtn = this.querySelector('.paypal-button-mini-cart');
          const cart_data = event.detail?.cart;
          if (checkoutBtn && cart_data) {
            initPaypalCheckoutBtn({
              btnClass: '.paypal-button-mini-cart',
              totalPrice: cart_data.total_price / 100,
            });
          }
        },
        ((_a = event == null ? void 0 : event.detail) == null ? void 0 : _a.replacementDelay) ||
  ...
  var LineItemQuantity = class extends CustomHTMLElement {
    ...
    async _updateFromLink(link) {
      ...
      if (window.themeVariables.settings.pageType === 'cart') {
        mainCart = document.querySelector('.shopify-section--main-cart');
        section = mainCart.querySelector('section');
        sectionId = section ? section.getAttribute('section-id') : null;
        section.querySelectorAll('.line-item .line-item__loader').forEach((item) => {
          if (!item) return;
          item.hidden = false;
      ...
      if (window.themeVariables.settings.pageType === 'cart') {
        const response = await fetch(`${window.themeVariables.routes.rootUrl}?section_id=${sectionId}`, {
          credentials: 'same-origin',
          method: 'GET',
        });
        const cartContent = await response.text();
        if (mainCart) mainCart.outerHTML = cartContent;
        return;
      }
  ...
  // js/custom-element/section/countdown/countdown.js
  var ZalifyCountdown = class extends CustomHTMLElement {
    connectedCallback() {
      this._run();
    }
    ...
    _run() {
      const { rootId, initialEndTime } = this.dataset;
      var rootEl = document.getElementById(rootId);
      if (!rootEl) {
        return;
      }
      var endTime = new Date(rootEl.dataset.endTime || initialEndTime).getTime();
      if (isNaN(endTime)) {
        return;
      }
      ...
      if (timeLeft > 0) {
        ...
        var secs = this._transformNumber(timeLeft % 60);
        var mins = this._transformNumber(Math.floor(timeLeft / 60) % 60);
        var hours = this._transformNumber(Math.floor(timeLeft / (60 * 60)) % 24);
        var days = this._transformNumber(Math.floor(timeLeft / (60 * 60 * 24)));

        var numberItems = rootEl.querySelectorAll('.countdown-number');
        if (numberItems[0].innerHTML !== days) {
          numberItems[0].innerHTML = days;
        }
        if (numberItems[2].innerHTML !== hours) {
          numberItems[2].innerHTML = hours;
        }
        if (numberItems[4].innerHTML !== mins) {
          numberItems[4].innerHTML = mins;
        }
        if (numberItems[6].innerHTML !== secs) {
          numberItems[6].innerHTML = secs;
          rootEl.setAttribute('serial-secs', secs);
        }
  ...
  window.customElements.define('zalify-countdown', ZalifyCountdown);
  ...
  var ProductStickyForm = class extends HTMLElement {
    connectedCallback() {
      ...
      this.priceElement2 = this.querySelector('.product-sticky-form__priceCom');
    ...
    _onVariantChanged(event) {
      ...
      if (this.priceElement) {
        this.priceElement.innerHTML = formatMoney(variant['price']);
      }
      if (this.priceElement2) {
        this.priceElement2.innerHTML = formatMoney(variant['compare_at_price']);
      }
    ...
    _setupVisibilityObservers() {
      const paymentContainerElement = document.getElementById('MainPaymentContainer'),
        ...
        stickyHeaderOffset = getStickyHeaderOffset();
      this._isPaymentContainerPassed = false;
      this.intersectionObserver = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (entry.target === paymentContainerElement) {
              const boundingRect = paymentContainerElement.getBoundingClientRect();
              this._isPaymentContainerPassed = entry.intersectionRatio == 0 && boundingRect.top < stickyHeaderOffset;
            }
          });
  ...
  var MiniCartRecommendations = class extends HTMLElement {
    connectedCallback() {
      this.pageDots = this.querySelector('page-dots');
      this.scrollBarElement = this.querySelector('.mini-cart__progress-bar');
      this.listWrapperElement = this.querySelector('.mini-cart__list-wrapper');
      this.listItemElements = Array.from(this.querySelectorAll('product-item'));
      this.isScrolling = false;
      if (this.listItemElements.length > 1) {
        this.addEventListener('prev-next:prev', this.previous.bind(this));
        this.addEventListener('prev-next:next', this.next.bind(this));
        this.addEventListener('page-dots:changed', (event) => this.select(event.detail.index));
        this.itemIntersectionObserver = new IntersectionObserver(this._onItemObserved.bind(this), { threshold: 0.4 });
        const mediaQuery = window.matchMedia(window.themeVariables.breakpoints.tabletAndUp);
        mediaQuery.addListener(this._onMediaChanged.bind(this));
        this._onMediaChanged(mediaQuery);
      }
    }
  ...
  window.customElements.define('mini-cart-recommendations', MiniCartRecommendations);

  // js/custom-element/section/product-list/product-list.js
  var ProductRecommendationList = class extends CustomHTMLElement {
    constructor() {
      super();
      this.productListInner = this.querySelector('.product-recommendation-list__inner');
      this.productItems = Array.from(this.querySelectorAll('product-item'));
    }
    connectedCallback() {
      this.addEventListener('prev-next:prev', this.previous.bind(this));
  ...
  __publicField(ProductList, 'observedAttributes', ['hidden']);
  window.customElements.define('product-recommendation-list', ProductRecommendationList);

  // Stories slider
  var StoriesSlider = class extends HTMLElement {
    connectedCallback() {
      this.listWrapperElement = this.querySelector('.stories-slider__wrapper');
      this.listItemElements = Array.from(this.querySelectorAll('.stories-slider__item'));
      this.isScrolling = false;
      if (this.listItemElements.length > 1) {
        if (Shopify.designMode) {
          this.addEventListener('shopify:block:select', (event) => {
            this.select(
              [...event.target.parentNode.parentNode.children].indexOf(event.target.parentNode),
              !event.detail.load,
            );
          });
        }
  ...
  window.customElements.define('stories-slider', StoriesSlider);

  // Back to top
  var BackToTop = class extends CustomHTMLElement {
    connectedCallback() {
      this.masterElement = this.querySelector('button');
      if (!this.masterElement) return;

      this.masterElement.addEventListener('click', (event) => {
        event.preventDefault();
        event.stopPropagation();
        window.scrollTo({
          top: 0,
      ...
      if (Shopify.designMode) {
        this.rootDelegate.on('shopify:section:select', (event) => {
          if (event.detail.sectionId === 'back-to-top') {
            this.masterElement.setAttribute('force-visible', '');
          } else {
            this.masterElement.removeAttribute('force-visible');
          }
        });
      }
    }
    _trackScroll() {
      ...
      const coords = this.dataset.verticalOffset;
      if (scrolled > coords) {
        this.masterElement.removeAttribute('hidden');
      }
      if (scrolled < coords) {
        this.masterElement.setAttribute('hidden', '');
      }
    }
  };
  ...
  var FreeGiftProductPopup = class extends ModalContent {
    constructor() {
      super();
      this.productQuantity = 0;
      this.drawerEl = null;
      this.sections = null;
      document.addEventListener('product:free-gift:show', (event) => {
        this.sections = event.detail.sections;
        this.drawerEl = event.detail.drawerEl;
        this.productQuantity = Number(event.detail.quantity);
        if (this.fixedQuantity) {
          this.productQuantity = 1;
        }
        this.open = true;
      });
      document.addEventListener('product:free-gift:added', (event) => {
        document.documentElement.dispatchEvent(
          new CustomEvent('cart:show-and-fetch', {
            bubbles: true,
            detail: {
              drawerEl: this.drawerEl,
    ...
    get fixedQuantity() {
      return this.hasAttribute('fixed-quantity');
    }
    get quantity() {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      switch (name) {
        case 'open':
          if (this.open) {
            this.querySelector('.product-free-gift-popup__quantity').value = this.productQuantity * this.quantity;
          } else {
            this.querySelector('.product-free-gift-popup__quantity').value = this.quantity;
          }
      }
    }
  };
  window.customElements.define('free-gift-product-popup', FreeGiftProductPopup);

  // Free gift wrapper
  var FreeGiftWrapper = class extends CustomHTMLElement {
    connectedCallback() {
      if (document.querySelector('[block-type="free-gift"]')) {
        const productForm = this.closest('.product-form');
        this.innerHTML = '';
        this.innerHTML = document.querySelector('[block-type="free-gift"]').outerHTML;
        productForm.querySelector('input[name="free_gift_product_id"]').outerHTML = document.querySelector(
          'input[name="free_gift_product_id"]',
        ).outerHTML;
        productForm.querySelector('input[name="free_gift_product_quantity"]').outerHTML = document.querySelector(
          'input[name="free_gift_product_quantity"]',
        ).outerHTML;
        productForm.querySelector('input[name="free_gift_type"]').outerHTML =
          document.querySelector('input[name="free_gift_type"]').outerHTML;
      }
    }
  };
  window.customElements.define('free-gift-wrapper', FreeGiftWrapper);

  // Cart apply code
  var CartApplyCode = class extends CustomHTMLElement {
    connectedCallback() {
      this.inputElement = this.querySelector('input[name="discount"]');
      this.applyButton = this.querySelector('button[name="apply"]');

      if (this.applyButton) {
        this.applyButton.addEventListener('click', () => {
          this.applyButton.setAttribute('disabled', 'disabled');
          this.applyButton.setAttribute('aria-busy', 'true');
          this._onApply().catch((err) => {
            this.applyButton.removeAttribute('disabled');
            this.applyButton.removeAttribute('aria-busy');
          });
        });
      } else {
        this.inputElement.addEventListener('input', this._onValueInput.bind(this));
      }
    }
    async _onApply() {
      const code = this.applyButton.value;
    ...
    _onValueInput(e) {
      document.querySelector('#mini-cart-form input[name="discount"]').value = e.target.value;
    }
  };
  window.customElements.define('cart-apply-code', CartApplyCode);

  // Product bundles
  const ProductBundles = class extends HTMLElement {
    constructor() {
      super();
      this.formId = this.getAttribute('form-id');
    }
    connectedCallback() {
      const bundleBtns = this.querySelectorAll('.product-form__bundle--item');
      if (bundleBtns && bundleBtns.length) {
        bundleBtns.forEach((btn) => {
          btn.addEventListener('click', (event) => {
            event.stopPropagation();
            this._handleAddToCart(event);
          });
        });
      }
    }
    async _handleAddToCart(event) {
      ...
      const atcBtn = formElement.querySelector('[data-product-add-to-cart-button]');
      const quantity = event.target.dataset.quantity;
      const inputElement = document.querySelector(`.product-form .quantity-selector__input[form="${this.formId}"]`);
      const rawQuantity = inputElement.quantity;

      if (quantity && inputElement) {
        inputElement.quantity = quantity;
        await atcBtn.click();
        inputElement.quantity = rawQuantity;
      }
    }
  };
  window.customElements.define('product-bundles', ProductBundles);
  ...
  (() => {
    document.addEventListener('variant:changed', async function (event) {
      ...
      const bundleElement = formId ? document.querySelector(`.product-form__bundle[form-id="${formId}"]`) : null;
      const bundleWrapper = bundleElement ? bundleElement.closest('.product-form__bundles') : null;

      if (!variant || !bundleElement) return;
      if (!variant.available) {
        bundleWrapper.setAttribute('hidden', '');
        return;
      }
      bundleWrapper.removeAttribute('hidden');
      ...
      const response = await fetch(url.toString());
      const fakeDiv = document.createElement('div');
      fakeDiv.innerHTML = await response.text();
      bundleElement.outerHTML = fakeDiv.querySelector(`.product-form__bundle[form-id="${formId}"]`).outerHTML;
    });
  })();
  (() => {
    document.addEventListener('cart:show-and-fetch', async function (event) {
      ...
      cartContent['sections'] = event.detail['sections'];
      const { buttonEl, drawerEl } = event.detail;
      document.documentElement.dispatchEvent(
        new CustomEvent('cart:refresh', {
          bubbles: true,
          detail: {
            cart: cartContent,
            openMiniCart: window.themeVariables.settings.cartType === 'drawer' && drawerEl === null,
          },
        }),
      );
      if (buttonEl) {
  ...
  (() => {
    document.addEventListener('variant:changed', async function (event) {
      const variant = event.detail.variant;
      const formId = event.target.getAttribute('id');
      const paypalCheckoutContainer = formId
        ? document.querySelector(`.paypal-button-container[form-id="${formId}"]`)
        : null;

      if (!variant || !paypalCheckoutContainer) return;
      if (!variant.available) {
        paypalCheckoutContainer.setAttribute('hidden', '');
        return;
      }

      paypalCheckoutContainer.removeAttribute('hidden');
      paypalCheckoutContainer.innerHTML = '';
      initPaypalCheckoutBtn({
        btnClass: `.paypal-button-container[form-id="${formId}"]`,
        totalPrice: variant.price / 100,
      });
    });
  })();
})();
...
function initPaypalCheckoutBtn({ btnClass, totalPrice }) {
  const pageType = window.themeVariables.settings.pageType;
  if (pageType !== 'cart' && pageType !== 'product') return;
  if (!paypal || !btnClass) return;
...
class ToggleNavigation extends HTMLElement {
  connectedCallback() {
    ...
    this.mbArrows = this.querySelector('.mb-arrows');
    this.selectLanguage = this.querySelectorAll('li');
    this.handleClick(this.btn);
  ...
  findRegion(arr, language) {
    ...
    for (const item of arr) {
      if (Object.keys(item).includes(language)) {
        return `${item[language]}${pathname}#redirect`;
      }
    }
    return pathname;
  }
